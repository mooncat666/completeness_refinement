...
if [[ -z ${cons} ]]; # set -c (--consensus) option in pggb smoothxg to default (100) if it is not defined
then
cons=100
fi
...
srun -p htc-el8 --mem 32000 -c 1 -t 600 python ${path}/0_get_data_glo.py "${species}" $database
conda deactivate
...
# first bork module load & check for errors
module load gunc || { source ./bashrc ; module load gunc; } || { echo "ERROR: Module loading failed (gunc). Exiting" >> /dev/stderr ; exit 1; }

mkdir -p 1.1_gunc_refined/${species}/gunc

# run gunc
srun -p htc-el8 -n 1 -c 16 -t 2000 --mem 64000 gunc run -d 0_data/${species}/unzipped_init_MAGs -o 1.1_gunc_refined/${species}/gunc -e .fa -t 16 
...
srun -p htc-el8 -n 1 -c 16 -t 2000 --mem 64000 gunc run -d 1.1_gunc_refined/${species}/gunc/refinement/refined_genomes/${gunc_clade} -o 1.1_gunc_refined/${species}/gunc/refinement/refined_genomes/${gunc_clade}/gunc -e .fa -t 16
...
srun -p htc-el8 --mem 64000 -n 1 -c 16 -t 1200 dedupe.sh threads=16 in=1.2_dedupl/${species}/merged.fa out=1.2_dedupl/${species}/dd_merged.fa minidentity=99 exact=f


# 1.3 discard short contigs from assemblies
log 1.3 discard short contigs from assemblies
...
srun -p htc-el8 -n 1 -c 16 -t 6000 --mem 200000 pggb -i 1.2_dedupl/${species}/dd_merged.fa -o 2.1_build_vg/${species} -P asm20 -t 16 -p 94 -s 10000 -C cons,${cons} -n ${files_number} -k 49 -G 7919,8069 --skip-viz
...
if [ ! -f 2.1_build_vg/${species}/*cons\@${cons}*.gfa ] && [ ! -f 2.1_build_vg/${species}/*final.gfa ]; then
  if [ -f 2.1_build_vg/${species}/*.paf ]; then
    srun -p htc-el8 -n 1 -c 16 -t 6000 --mem 256000 pggb -a 2.1_build_vg/${species}/*.paf -i 1.2_dedupl/${species}/dd_merged.fa -o 2.1_build_vg/${species} -P asm20 -t 16 -p 94 -s 10000 -C cons,${cons} -n ${files_number} -k 49 -G 7919,8069 --skip-viz
  else
    srun -p htc-el8 -n 1 -c 16 -t 6000 --mem 256000 pggb -i 1.2_dedupl/${species}/dd_merged.fa -o 2.1_build_vg/${species} -P asm20 -t 16 -p 94 -s 10000 -C cons,${cons} -n ${files_number} -k 49 -G 7919,8069 --skip-viz
  fi
fi
...
log '2.2 remove nodes uncovered by initial contigs from variation graph'
...
srun -p htc-el8 -n 1 --mem 64000 -t 4000 GraphAligner --multimap-score-fraction 0 --min-alignment-score 100 -a 2.2_edit_vg/${species}/mapped_renamed_pulled_initMAGs.gaf -x vg -f 2.2_edit_vg/${species}/renamed_pulled_init_MAGs.fa -g ${graph} --precise-clipping 0.95
conda deactivate
...
conda activate pggb
srun -p htc-el8 -n 1 -t 600 --mem 32000 odgi extract -i ${graph} -o 2.2_edit_vg/${species}/filtered_graph.og -l 2.2_edit_vg/${species}/filtered_nodes.txt
srun -p htc-el8 -n 1 -t 600 --mem 32000 odgi view -i 2.2_edit_vg/${species}/filtered_graph.og -g > 2.2_edit_vg/${species}/filtered_graph.gfa
conda deactivate
...
log '3.1 and 3.2 map contigs from assemblies to graph'
...
submitjob -s SLURM -n ${species:0:4} -r '-p htc-el8' -c 4 -m 64 -a 100 -k 48:00:00 3.1_map_contigs/${species}/jobs_map.sh | (read job_id ; python ${path}/check_job_status.py $job_id ; echo $job_id > 3.1_map_contigs/${species}/job_id.txt )
...
log '4.1 gunc refine mapped bins'
...
# run gunc
srun -p htc-el8 -n 1 -c 16 -t 2000 --mem 64000 gunc run -d 3.2_filter_alignments/${species} -o 4.1_refine_output/${species}/gunc -e .fa -t 16
...
module purge
module load gunc
srun -p htc-el8 -n 1 -c 16 -t 2000 --mem 64000 gunc run -d . -o gunc -e .fa -t 16
module purge

# run checkm2
module load checkm2
srun -p htc-el8 -n 1 -c 32 --mem 64000 -t 1200 checkm2 predict -i .  -t 32 -o checkm2 -x .fa
module purge
